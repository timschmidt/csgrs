...
fn test_indexed_mesh_shapes_equivalence() {
    ...
    println!("IndexedMesh cube faces: {}, Regular cube faces: {}",
             indexed_cube.polygons.len(), regular_cube.polygons.len());
...
fn test_indexed_mesh_sphere() {
    ...
    for vertex in &indexed_sphere.vertices {
        ...
        assert!((distance_from_origin - radius).abs() < 0.1, 
                "Vertex distance {} should be close to radius {}", distance_from_origin, radius);
    }
    ...
    println!("Sphere manifold analysis: boundary_edges={}, non_manifold_edges={}, polygons={}",
             manifold_analysis.boundary_edges, manifold_analysis.non_manifold_edges, indexed_sphere.polygons.len());
    // For now, just check that it has reasonable structure (boundary edges are expected for subdivided spheres)
    assert!(manifold_analysis.connected_components > 0, "Should have at least one connected component");
...
fn test_indexed_mesh_cylinder() {
    ...
    assert!((width - 2.0 * radius).abs() < 0.1, "Cylinder width should be 2*radius");
    assert!((depth - 2.0 * radius).abs() < 0.1, "Cylinder depth should be 2*radius");
    assert!((mesh_height - height).abs() < 0.1, "Cylinder height should match input");
...
fn test_indexed_mesh_manifold_validation() {
    ...
    assert!(analysis.is_manifold, "Manifold analysis should confirm cube is manifold");
    assert_eq!(analysis.boundary_edges, 0, "Cube should have no boundary edges");
...
fn test_indexed_mesh_quality_analysis() {
    ...
    assert!(!quality_metrics.is_empty(), "Should have quality metrics for triangles");

    // Check that all triangles have reasonable quality
    let min_quality = quality_metrics.iter().map(|q| q.quality_score).fold(1.0, f64::min);
    let avg_quality = quality_metrics.iter().map(|q| q.quality_score).sum::<f64>() / quality_metrics.len() as f64;
    let degenerate_count = quality_metrics.iter().filter(|q| q.area < Real::EPSILON).count();

    assert!(min_quality > 0.3, "Cube triangles should have reasonable quality");
...
fn test_indexed_mesh_flattening() {
    ...
    assert!(!flattened.geometry.0.is_empty(), "Flattened geometry should not be empty");
...
fn test_indexed_mesh_sdf_generation() {
    ...
    assert!(!sdf_sphere.vertices.is_empty(), "SDF sphere should have vertices");
    assert!(!sdf_sphere.polygons.is_empty(), "SDF sphere should have faces");
    ...
    assert!(surface_ratio > 0.8, "Most vertices should be on sphere surface");
...
fn test_indexed_mesh_convex_hull() {
    ...
    let cube = IndexedMesh::<()>::cube(1.0, None);
    let hull = cube.convex_hull().expect("Convex hull computation should succeed");
...
fn test_indexed_mesh_metaballs() {
    ...
    assert!(!metaball_mesh.vertices.is_empty(), "Metaball mesh should have vertices");
    assert!(!metaball_mesh.polygons.is_empty(), "Metaball mesh should have faces");
...
fn test_indexed_mesh_tpms() {
    ...
    println!("Gyroid manifold analysis: is_manifold={}, boundary_edges={}, non_manifold_edges={}",
    ...
    assert!(analysis.connected_components > 0, "Gyroid should have connected components");
...
fn test_indexed_mesh_memory_efficiency() {
    ...
    println!("  Regular Mesh vertex instances: {}", regular_cube.total_vertex_count());
}
...
fn test_indexed_mesh_no_conversion_no_open_edges() {
    ...
    println!("Union result: vertices={}, polygons={}, boundary_edges={}",
             union_result.vertices.len(), union_result.polygons.len(), union_analysis.boundary_edges);
    println!("Difference result: vertices={}, polygons={}, boundary_edges={}",
             difference_result.vertices.len(), difference_result.polygons.len(), difference_analysis.boundary_edges);
    println!("Intersection result: vertices={}, polygons={}, boundary_edges={}",
             intersection_result.vertices.len(), intersection_result.polygons.len(), intersection_analysis.boundary_edges);
    ...
    assert!(union_analysis.boundary_edges == 0 || union_analysis.boundary_edges < union_result.polygons.len(),
            "Union should have reasonable boundary structure");
    assert!(difference_analysis.boundary_edges == 0 || difference_analysis.boundary_edges < difference_result.polygons.len(),
            "Difference should have reasonable boundary structure");

    // Test that IndexedMesh preserves vertex sharing efficiency
    let total_vertex_references = union_result.polygons.iter().map(|p| p.indices.len()).sum::<usize>();
    let unique_vertices = union_result.vertices.len();
    let sharing_ratio = total_vertex_references as f64 / unique_vertices as f64;

    println!("Vertex sharing efficiency: {} references / {} unique = {:.2}x",
