...
impl PlaneJs {
    ...
    pub fn to_xy_transform(&self) -> JsValue {
        ...
        let to_xy_flat: Vec<f64> = to_xy.as_slice().iter().copied().map(|v| v as f64).collect();
        let from_xy_flat: Vec<f64> = from_xy.as_slice().iter().copied().map(|v| v as f64).collect();

        let obj = js_sys::Object::new();
        js_sys::Reflect::set(&obj, &"toXY".into(), &js_sys::Float64Array::from(to_xy_flat.as_slice())).unwrap();
        js_sys::Reflect::set(&obj, &"fromXY".into(), &js_sys::Float64Array::from(from_xy_flat.as_slice())).unwrap();
