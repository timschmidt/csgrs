...
impl<T: MixedNum + MixedOps + Copy> Vec3<T> {
    #[inline]
...
impl<T: MixedNum + MixedOps + Copy> Add for Vec3<T> { type Output = Self; fn add(self, r: Self) -> Self { Self::new(self.x+r.x, self.y+r.y, self.z+r.z) } }
impl<T: MixedNum + MixedOps + Copy> Sub for Vec3<T> { type Output = Self; fn sub(self, r: Self) -> Self { Self::new(self.x-r.x, self.y-r.y, self.z-r.z) } }
impl<T: MixedNum + MixedOps + Copy> Neg for Vec3<T> { type Output = Self; fn neg(self) -> Self { Self::new(-self.x, -self.y, -self.z) } }
impl<T: MixedNum + MixedOps + Copy> Mul<T> for Vec3<T> { type Output = Self; fn mul(self, s: T) -> Self { Self::new(self.x*s, self.y*s, self.z*s) } }
impl<T: MixedNum + MixedOps + Copy> Div<T> for Vec3<T> { type Output = Self; fn div(self, s: T) -> Self { Self::new(self.x/s, self.y/s, self.z/s) } }
impl<T: MixedNum + MixedOps + Copy> AddAssign for Vec3<T> { fn add_assign(&mut self, r: Self) { self.x+=r.x; self.y+=r.y; self.z+=r.z; } }
impl<T: MixedNum + MixedOps + Copy> SubAssign for Vec3<T> { fn sub_assign(&mut self, r: Self) { self.x-=r.x; self.y-=r.y; self.z-=r.z; } }
impl<T: MixedNum + MixedOps + Copy> MulAssign<T> for Vec3<T> { fn mul_assign(&mut self, s: T) { self.x*=s; self.y*=s; self.z*=s; } }
impl<T: MixedNum + MixedOps + Copy> DivAssign<T> for Vec3<T> { fn div_assign(&mut self, s: T) { self.x/=s; self.y/=s; self.z/=s; } }
...
impl<T: MixedNum + MixedOps + Copy> Point3<T> {
    ...
    #[inline]
    pub fn to_vec(self) -> Vec3<T> { Vec3::new(self.x, self.y, self.z) }
}

impl<T: MixedNum + MixedOps + Copy> Add<Vec3<T>> for Point3<T> {
    type Output = Self;
    fn add(self, v: Vec3<T>) -> Self { Self::new(self.x+v.x, self.y+v.y, self.z+v.z) }
}
impl<T: MixedNum + MixedOps + Copy> Sub<Vec3<T>> for Point3<T> {
    type Output = Self;
    fn sub(self, v: Vec3<T>) -> Self { Self::new(self.x-v.x, self.y-v.y, self.z-v.z) }
}
impl<T: MixedNum + MixedOps + Copy> Sub<Point3<T>> for Point3<T> {
    type Output = Vec3<T>;
    fn sub(self, p: Point3<T>) -> Vec3<T> { Vec3::new(self.x-p.x, self.y-p.y, self.z-p.z) }
}
...
impl<T: MixedNum + MixedOps + Copy> Mat3<T> {
    #[inline]
    pub fn from_rows(rows: [[T; 3]; 3]) -> Self { Self { m: rows } }

    #[inline]
    pub fn from_cols(cols: [Vec3<T>; 3]) -> Self {
        Self::from_rows([
    ...
    #[inline]
    pub fn rotation_axis_angle(axis: Vec3<T>, angle_rad: T) -> Self
    ...
    pub fn rotation_between(from: Vec3<T>, to: Vec3<T>) -> Self
    ...
    /// R = I - 2 * n nᵀ
    pub fn reflection_about_unit_normal(n: Vec3<T>) -> Self
...
impl<T: MixedNum + MixedOps + Copy> Mat4<T> {
    #[inline]
    pub fn identity() -> Self
    where T: MixedZero + MixedOne
    {
    ...
    #[inline]
    pub fn from_mat3_and_translation(r: Mat3<T>, t: Vec3<T>) -> Self
    where T: MixedZero + MixedOne
    {
        Self {
            m: [
    ...
    #[inline]
    pub fn translation(v: Vec3<T>) -> Self where T: MixedZero + MixedOne {
        Self::from_mat3_and_translation(Mat3::identity(), v)
    }
    ...
    where T: MixedTrigonometry + MixedZero + MixedOne
    {
        Self::from_mat3_and_translation(Mat3::rotation_x(angle_rad), Vec3::zeros())
    }
    ...
    where T: MixedTrigonometry + MixedZero + MixedOne
    {
        Self::from_mat3_and_translation(Mat3::rotation_y(angle_rad), Vec3::zeros())
    }
    ...
    where T: MixedTrigonometry + MixedZero + MixedOne
    {
        Self::from_mat3_and_translation(Mat3::rotation_z(angle_rad), Vec3::zeros())
    }
    ...
    where T: MixedTrigonometry + MixedZero + MixedOne
    {
        Self::from_mat3_and_translation(Mat3::rotation_axis_angle(axis, angle_rad), Vec3::zeros())
    }
    ...
    where T: MixedZero + MixedOne + MixedNumConversion<f64>
    {
        Self::from_mat3_and_translation(Mat3::rotation_between(from,to), Vec3::zeros())
    }
    ...
    #[inline]
    pub fn transform_normal(self, n: Vec3<T>) -> Vec3<T>
    where T: MixedZero + MixedOne + MixedNumConversion<f64>
    {
        let r = self.upper_left_3x3();
        if let Some(inv) = r.try_inverse() {
            inv.transpose().mul_vec3(n).normalize()
        } else {
    ...
    #[inline]
    pub fn upper_left_3x3(self) -> Mat3<T> {
        Mat3::from_rows([
    ...
    where T: MixedZero + MixedOne + MixedNumConversion<f64>
    {
        ...
        let t_inv = r_inv.mul_vec3(-t);

        Some(Self::from_mat3_and_translation(r_inv, t_inv))
    }
}
...
#[inline]
pub fn translate<T>(dx: T, dy: T, dz: T) -> Mat4<T>
...
#[inline]
pub fn scale<T>(sx: T, sy: T, sz: T) -> Mat4<T>
where T: MixedNum + MixedOps + MixedZero + MixedOne + Copy
{
...
#[inline]
pub fn rotate_xyz<T>(rx_rad: T, ry_rad: T, rz_rad: T) -> Mat4<T>
where T: MixedNum + MixedOps + MixedTrigonometry + MixedZero + MixedOne + Copy
{
    Mat4::rotation_z(rz_rad).mul(Mat4::rotation_y(ry_rad)).mul(Mat4::rotation_x(rx_rad))
}
...
pub struct Rotation3 {
    pub mat: Mat3, // right-handed 3×3 rotation matrix
}

impl Rotation3 {
    #[inline]
    pub fn identity() -> Self {
        Self { mat: Mat3::identity() }
    }
    ...
    pub fn from_axis_angle(axis: &Vec3, angle: Real) -> Self {
        ...
        Self { mat: Mat3::from_rows([[r00, r01, r02],
                                     [r10, r11, r12],
                                     [r20, r21, r22]]) }
    }
    ...
    pub fn rotation_between(a: &Vec3, b: &Vec3) -> Option<Self> {
        let la = a.norm();
        let lb = b.norm();
        ...
        if s <= EPSILON {
            ...
            if c > 0.0 {
                ...
                let mut pick = if u.x.abs() < u.y.abs() && u.x.abs() < u.z.abs() {
                    Vec3::new(Real::ONE, 0.0, 0.0)
                } else if u.y.abs() < u.z.abs() {
                    Vec3::new(0.0, Real::ONE, 0.0)
                } else {
                    Vec3::new(0.0, 0.0, Real::ONE)
                };
    ...
    pub fn to_homogeneous(&self) -> Mat4 {
        Mat4::from_rotation_translation(self.mat, Vec3::zero())
    }
}
...
impl From<Mat3> for Rotation3 {
    #[inline]
    fn from(mat: Mat3) -> Self { Self { mat } }
}
...
impl Translation3 {
    ...
    #[inline]
    pub fn to_homogeneous(&self) -> Mat4 {
        Mat4::from_rotation_translation(Mat3::identity(), self.v)
    }
}
...
impl Isometry3 {
    ...
    #[inline]
    pub fn to_homogeneous(&self) -> Mat4 {
        Mat4::from_rotation_translation(self.rotation.mat, self.translation.v)
    }

    /// Point transform: p' = R·p + t
    #[inline]
    pub fn transform_point(&self, p: &Point3<Real>) -> Point3<Real> {
        let r = self.rotation.mat * p.to_vec3();
