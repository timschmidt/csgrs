...
use core::fmt::Debug;
use crate::math_ndsp::{Point3, eps};
...
fn rings_of<S: Clone + Send + Sync + Debug, T>(sk: &Sketch<S, T>) -> Vec<Vec<(Real, Real)>> {
...
pub fn fdm_layer_from_sketch<S: Clone + Send + Sync + Debug, T>(
    region: &Sketch<S, T>,
    z: Real,
    cfg: &FdmLayerCfg,
    feeds: &Feeds,
...
pub fn cut2d_contours<S: Clone + Send + Sync + Debug, T>(
    region: &Sketch<S, T>,
    z: Real,
    kerf: Real,
    side: KerfSide,
    ...
    for ring in rings_of(&comp) {
        ...
        let ux = if len > eps::<T>() { dir.0 / len } else { 1.0 };
        let uy = if len > eps::<T>() { dir.1 / len } else { 0.0 };
...
pub fn pocket2d<S: Clone + Send + Sync + Debug, T>(
    region: &Sketch<S, T>,
    z_safety: Real,
    ...
    #[cfg(feature = "offset")]
    let radial_step = cfg.stepover.max(eps::<T>()) * cfg.tool.diameter; // fraction * D
    #[cfg(not(feature = "offset"))]
...
pub fn lathe_rough_from_profile<S: Clone + Send + Sync + Debug, T>(
    profile_xy: &Sketch<S, T>,
    z_min: Real,
    z_max: Real,
    stock_radius: Real,
    cfg: &LatheCfg,
...
pub fn contour_only<S: Clone + Send + Sync + Debug, T>(
    sk: &Sketch<S, T>,
    z: Real,
    feed: Real,
    kind: MachineKind,
