//! Sample metaball fields into native [`hypermesh::TriangleMesh`] geometry.
...
/// Creates native triangle geometry from metaballs by sampling a 3D grid.
///
...
pub fn metaballs(
    ...
    metaballs_with_diagnostics(balls, resolution, iso_value, padding).0
}

/// Creates native triangle geometry from metaballs and returns sampling and
...
/// composes the sampled surface directly into Hypermesh triangle geometry while
...
pub fn metaballs_with_diagnostics(
    ...
    let (Ok(requested_nx), Ok(requested_ny), Ok(requested_nz)) = (
        u32::try_from(resolution.0.max(2)),
        u32::try_from(resolution.1.max(2)),
        u32::try_from(resolution.2.max(2)),
    ) else {
        return (
    ...
    let Some(requested_sample_count) = requested_nx
        .checked_mul(requested_ny)
        .and_then(|count| count.checked_mul(requested_nz))
    else {
        return (
            TriangleMesh::new(Vec::new(), Vec::new()),
            MetaballDiagnostics {
                resolution: (requested_nx, requested_ny, requested_nz),
                ball_count: balls.len(),
                ..MetaballDiagnostics::default()
            },
        );
    };
    let mut nx = requested_nx;
    let mut ny = requested_ny;
    let mut nz = requested_nz;
    let mut diagnostics = MetaballDiagnostics {
        resolution: (nx, ny, nz),
        ball_count: balls.len(),
        sample_count: requested_sample_count as usize,
        ..MetaballDiagnostics::default()
    };
    ...
    // Sample one proven-exterior cell around the requested grid so standalone
    // metaballs are closed rather than behaving like an uncapped chunk.
    let (Some(bordered_nx), Some(bordered_ny), Some(bordered_nz)) = (
        requested_nx.checked_add(2),
        requested_ny.checked_add(2),
        requested_nz.checked_add(2),
    ) else {
        return (TriangleMesh::new(Vec::new(), Vec::new()), diagnostics);
    };
    let Some(bordered_sample_count) = bordered_nx
        .checked_mul(bordered_ny)
        .and_then(|count| count.checked_mul(bordered_nz))
    else {
        return (TriangleMesh::new(Vec::new(), Vec::new()), diagnostics);
    };
    nx = bordered_nx;
    ny = bordered_ny;
    nz = bordered_nz;
    diagnostics.resolution = (nx, ny, nz);
    diagnostics.sample_count = bordered_sample_count as usize;
    ...
    // Preserve Surface Nets' shared vertex buffer. Expanding every triangle
    // into three private positions makes a geometrically closed surface fail
    // every indexed-manifold query.
    ...
    for tri in sn_buffer.indices.chunks_exact(3) {
        ...
        // Surface Nets indices.
        triangles.push(Triangle::new(i0, i2, i1));
...
fn metaball_bounds_hreal(
    ...
    // If every ball contributes less than iso / n, their sum is outside.
    // A small exact safety factor keeps the extracted zero set strictly away
    // from the outermost samples even for a single ball and zero padding.
    let count_over_iso = (Real::from(balls.len() as u64) / iso_value.clone()).ok()?;
    let influence_extent_scale =
        count_over_iso.sqrt().ok()? * Real::from(101_u8) / Real::from(100_u8);
    let influence_extent_scale = influence_extent_scale.ok()?;
    let mut bounds = balls.iter().map(|ball| {
        ...
        let extent = radius * influence_extent_scale.clone() + padding.clone();
        Some((
            HPoint3::new(
...
fn add_surface_nets_border(
    ...
    let step = |min: &Real, max: &Real, count: u32| {
        ((max.clone() - min.clone()) / Real::from(u64::from(count - 1))).ok()
    };
    let dx = step(&min_pt.x, &max_pt.x, nx)?;
    let dy = step(&min_pt.y, &max_pt.y, ny)?;
    let dz = step(&min_pt.z, &max_pt.z, nz)?;
    Some((
        HPoint3::new(
            min_pt.x - dx.clone(),
            min_pt.y - dy.clone(),
        ...
        HPoint3::new(max_pt.x + dx, max_pt.y + dy, max_pt.z + dz),
    ))
}
...
mod tests {
    use super::*;
    use crate::hyper_math::{hreal_from_f64, hreal_gt_f64, tolerance};
