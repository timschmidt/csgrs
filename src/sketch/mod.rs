...
#[derive(Debug, Clone)]
/// String of line points for rendering in 3D space
pub struct GraphicLineString {
    pub points: Vec<[f32; 3]>,
}

impl GraphicLineString {
    /// Get the total number of individual lines that can be made with the
    /// line string
...
#[derive(Debug, Clone)]
/// Collection of independent graphic line strings
pub struct GraphicLineStrings {
    pub line_strings: Vec<GraphicLineString>,
}

impl GraphicLineStrings {
    /// Get the total number of individual lines that can be made with all of
    /// the line strings. Iterates through each line string stored so can get
    /// expensive
    pub fn total_line_count(&self) -> usize {
        let mut total_line_count = 0;

        for line_string in &self.line_strings {
            total_line_count += line_string.line_count();
        }
...
pub struct Sketch<S> {
    ...
    /// Not public due to needing to update the origin transform each time
    /// the origin is set.
    pub(crate) origin: Vertex,

    pub(crate) origin_transform: OriginTransformVecQuat,
}
...
impl<S: Clone + Send + Sync + Debug> Sketch<S> {
    ...
    #[inline]
    /// Set the origin of the sketch.
    pub fn set_origin(&mut self, origin: Vertex) {
        self.origin_transform = Self::prepare_origin_vec_and_quat(origin);
        self.origin = origin;
    }
    ...
    /// Prepares the vector and quaternion needed to move all the points of
    /// a mesh generated by this sketch to the origin
    pub(crate) fn prepare_origin_vec_and_quat(origin: Vertex) -> OriginTransformVecQuat {
        ...
        let rotation_quat =
            match UnitQuaternion::rotation_between(&default_origin.normal, &origin.normal) {
                ...
                None => {
                    ...
                    let quat1 = UnitQuaternion::rotation_between(
                        &default_origin.normal,
                        &inter_origin,
                    )
                    .unwrap();
                    let quat2 =
            ...
            match geometry {
                Geometry::Polygon(polygon) => {
                    graphic_line_strings
                        .push(self.line_string_to_graphic_line_string(polygon.exterior()));
                },
                Geometry::Line(line) => {
                    let start_point =
                        Point3::new(line.start.x as f32, line.start.y as f32, 0.0);
                    let end_point = Point3::new(line.end.x as f32, line.end.y as f32, 0.0);
                    let graphic_line_string = GraphicLineString {
                        points: vec![
                            start_point.coords.cast::<f32>().into(),
                            end_point.coords.cast::<f32>().into(),
                        ],
                    };

                    graphic_line_strings.push(graphic_line_string)
                },
                ...
                Geometry::GeometryCollection(geometry_collection) => {
                    let child_sketch = Self {
                        geometry: geometry_collection.clone(),
                    ...
                    let line_strings = child_sketch.build_graphic_line_strings();
    ...
    fn lines_to_line_string(lines: &[Line<f64>]) -> LineString {
        ...
        coord_vec.push(first_line.end);

        for line in lines_iter {
            coord_vec.push(line.end);
        }
    ...
    fn line_string_to_graphic_line_string(
        ...
        let points_vec = line_string
            .coords()
            .map(|coord| {
                let point = Point3::new(coord.x, coord.y, 0.0);

                let point_transformed =
                    apply_origin_transform_graphicpoint!(point, self.origin_transform);

                let graphics_point: [f32; 3] = point_transformed.coords.cast::<f32>().into();

                graphics_point
            })
            .collect();

        let graphic_line_string = GraphicLineString { points: points_vec };
...
impl<S: Clone + Send + Sync + Debug> CSG for Sketch<S> {
    ...
    fn new() -> Self {
        Sketch {
            ...
            origin_transform: Self::prepare_origin_vec_and_quat(Vertex::default()),
        }
    }
...
impl<S: Clone + Send + Sync + Debug> From<Mesh<S>> for Sketch<S> {
    fn from(mesh: Mesh<S>) -> Self {
        ...
        Sketch {
            ...
            metadata: None,
            origin: mesh.origin,
            origin_transform: Self::prepare_origin_vec_and_quat(mesh.origin),
        }
    }
}
