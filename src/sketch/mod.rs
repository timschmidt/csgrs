...
impl<S: Clone + Send + Sync + Debug> Sketch<S> {
    ...
    pub(crate) fn prepare_origin_vec_and_quat(origin: Vertex) -> OriginTransformVecQuat {
        ...
        let rotation_quats = match UnitQuaternion::rotation_between(&default_origin.normal, &origin.normal) {
            Some(quat) => (quat, None),
            None => {
                // Assumes the default normal is +Z. We'll do +Y
                let inter_origin = Vector3::new(0.0, 1.0, 0.0);

                let quat1 = UnitQuaternion::rotation_between(&default_origin.normal, &inter_origin).unwrap();
                let quat2 = UnitQuaternion::rotation_between(&inter_origin, &origin.normal).unwrap();
