...
impl<S: Clone + Debug + Send + Sync> IndexedMesh<S> {
    ...
    pub fn build_connectivity(&self) -> (VertexIndexMap, HashMap<usize, Vec<usize>>) {
        ...
        // First pass: build vertex index mapping from IndexedMesh vertices
        for vertex in &self.vertices {
            vertex_map.get_or_create_index(vertex.pos);
        }

        // Second pass: build adjacency graph using indexed polygons
        for polygon in &self.polygons {
            let vertex_indices = &polygon.indices;
