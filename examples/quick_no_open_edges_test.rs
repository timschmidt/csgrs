...
fn test_indexed_mesh_cube() {
    ...
    println!("   Vertices: {}, Polygons: {}", cube.vertices.len(), cube.polygons.len());
    println!("   Boundary edges: {}, Non-manifold edges: {}", 
...
fn test_indexed_mesh_sphere() {
    ...
    println!("   Vertices: {}, Polygons: {}", sphere.vertices.len(), sphere.polygons.len());
    println!("   Boundary edges: {}, Non-manifold edges: {}", 
             analysis.boundary_edges, analysis.non_manifold_edges);
...
fn test_indexed_mesh_cylinder() {
    ...
    println!("   Vertices: {}, Polygons: {}", cylinder.vertices.len(), cylinder.polygons.len());
    println!("   Boundary edges: {}, Non-manifold edges: {}", 
             analysis.boundary_edges, analysis.non_manifold_edges);
    
    // Cylinder may have some topology complexity due to end caps
    println!("   Is manifold: {}, Connected components: {}",
             analysis.is_manifold, analysis.connected_components);

    assert_eq!(analysis.boundary_edges, 0, "Cylinder should have no boundary edges");
    assert_eq!(analysis.non_manifold_edges, 0, "Cylinder should have no non-manifold edges");

    // For now, just check that it has reasonable structure
    assert!(analysis.connected_components > 0, "Cylinder should have connected components");
...
fn test_indexed_mesh_csg_operations() {
    ...
    println!("   Union - Vertices: {}, Polygons: {}, Boundary edges: {}", 
    ...
    println!("   Difference - Vertices: {}, Polygons: {}, Boundary edges: {}", 
             diff_result.vertices.len(), diff_result.polygons.len(), diff_analysis.boundary_edges);
    
    assert_eq!(diff_analysis.boundary_edges, 0, "Difference should have no boundary edges");
    assert_eq!(diff_analysis.non_manifold_edges, 0, "Difference should have no non-manifold edges");
    
    // Test intersection
    let intersect_result = cube1.intersection_indexed(&cube2);
    let intersect_analysis = intersect_result.analyze_manifold();
    println!("   Intersection - Vertices: {}, Polygons: {}, Boundary edges: {}", 
             intersect_result.vertices.len(), intersect_result.polygons.len(), intersect_analysis.boundary_edges);
    
    // Intersection may be empty (stub implementation)
    if !intersect_result.polygons.is_empty() {
        assert_eq!(intersect_analysis.boundary_edges, 0, "Intersection should have no boundary edges");
        assert_eq!(intersect_analysis.non_manifold_edges, 0, "Intersection should have no non-manifold edges");
    }
