...
if (import.meta.client)
{
  await loadWasm().then((csgrs) => 
  {
      ...
      let placeCursorX = 0;
      const curves = [
        csgrs.CurveRegionJs.polygon([[-20, -10],[20, -10],[15, 10],[-15, 20]]),
        csgrs.CurveRegionJs.rectangle(40, 20),
        csgrs.CurveRegionJs.circle(15, 32),
      ].map((curve, i, arr) =>
      {
          const curBbox = curve.boundingBox();
          const translatedCurveMesh = curve.translate(placeCursorX - curBbox.min[0], 0,0)

          placeCursorX += curBbox.max[0] - curBbox.min[0] + MARGIN;
          return translatedCurveMesh;
      });

      // Extrude in multiple ways
      const extrudedShapes = curves.map(
      (s,i) =>
      {
          if((i+1) % 2 === 0)
          {  
